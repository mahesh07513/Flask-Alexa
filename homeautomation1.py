...
def operateSwitchOn(switchname):
    ...
    if num==None:
        ...
        data={}
        if int(num) in num1:
            data = {'username':'mahesh','password':'mahesh12','switch_id':switchname+num,'operation':'ON'}
        else:
            data = {'username':'mahesh','password':'mahesh12','switch_id':switchname,'operation':'ON'}
...
@ask.launch
def start_skill():
    welcome_messege='Welcome to jochebed ,Which switch do you want to operae?'
...
def share_headlines():
    ...
    headlines = operateSwitch()
    headline_msg = 'the current world news headlines are {}'.format(headlines)
    return statement(headline_msg)

@ask.intent("NoIntent")
def no_intent():
    bye_text='i am not sure why you asked me to to run then , but okay ...bye'
    return statement(bye_text)

@ask.intent("OperateSwitchOn")
def on_intent():
	
    bye_text = 'this is on intent '
    intent = []
    print("intent 9 :"+int(intent['slots'])) 
    if nane in intent['slots']:
    	favorite_color = intent['slots']['light']['value']
    	if favorite_color in ['green', 'red', 'blue', 'orange','gold', 'silver', 'yellow','black', 'white']:
    		print("intent 1"+favorite_color)
    	else:
    		print("intent 2"+favorite_color)
    return statement(bye_text)

@ask.intent("OperateSwitchOff")
def off_intent():
    bye_text='this is off intent'
    return statement(bye_text)


@ask.intent("AddIntent",convert={'first':int,'second':int})
def add_intent(first,second):
	sum = first+second
	bye_text='{} plus {} is {}'.format(first,second,sum)
	return statement(bye_text)



@ask.intent("LikeIntent",convert={'preference':p2bool})
def like_intent(preference):
	
	if convert_errors:
		return statement("I heard you say {}".format(preference))
	if preference == True:
		return statement("you say like carrots")
	elif preference == False:
		return statement("carrots are not your favarite")
	raise Exception()



@ask.intent("SwitchOperate",convert={'switchname':get_details,'operation':get_operation})
def like_intent(switchname,operation):
    #print "operateSwitch :",switchname,operation
    print "execution 888888888888"
    swid[:]=[]
...
@ask.intent("SwitchOperate",convert={'switchname':get_details,'number':get_number,'operation':get_operation})
def like_intent(switchname,number,operation):
    #print "operateSwitch :",switchname,operation
    print "execution func11111111111111"
    swid[:]=[]
